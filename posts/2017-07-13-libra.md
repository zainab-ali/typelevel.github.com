...
Dimensional analysis can catch bugs in code which would otherwise result in incorrect calculations.
The most well known example of this is the Mars Climate Orbiter, which successfully reached Mars, but suddenly crashed into the Martian atmosphere when about to begin measurements.  Later investigations showed that the fault was a result of a unit conversion error.  Most components on the orbiter were using metric units, however a single component was sending instructions in Imperial units.  The other components did not detect this, and instead began a sudden descent.
...
The star that we're aiming for in the Libra constellation is Alpha Librae.  This is pretty far, so we can only send one very small person.  We have been given the following quantities:
...
Given these figures, we want to calculate when the rocket will arrive.
...
A represents the type of quantity. So given the following quantity types:
...
```scala
 def divide[A, B, Magic](q0: Quantity[A], q1: Quantity[B])(implicit d: Divide[A, B]): Quantity[d.Out] =
...
Great!  We've caught our bug!  The result was in `LightYearSecondsPerMetre`, not years.  We made a unit conversion error between lightyears and metres per second.

We can now fix this by adding a conversion:

```scala 
val metresPerSecondPerC = Quantity[MetresPerSecond](300000000.0).mps / Quantity[C](1.0)
...
We can represent a combination of dimensions as a hetrogenous list (HList) of base dimensions.  HLists represent a lists of types.  HLists are defined in [shapeless](https://github.com/milessabin/shapeless), and have become a cornerstone of most functional libraries.  They are modelled in a similar way to Scala Lists, with a head, tail and `::` constructor. 

```scala
type LightYearSeconds = LightYear :: Second :: HNil
```

This works for all multiples of dimension, such as metre seconds, but it doesn't represent combinations created from division, such as `MetresPerSecond`.
...
The exponents need to be part of the type signature, so they must be types, not values.  We can represent them using Singleton types:

```scala
val 3: Singleton with Int
```
We actually need these singleton types in type position.  This is supported by a new feature present in [Typelevel Scala](https://github.com/typelevel/scala), called [literal types](http://docs.scala-lang.org/sips/pending/42.type.html):
...
We can use a library called [singleton ops](<https://github.com/fthomas/singleton-ops>) to do this.  This provides us with type level integer operations:

```scala
OpInt.Aux[1 + 2, 3]
OpInt.Aux[3 * 2, 6]
```
This also gives us a conventient alias for singleton types

```scala
type XInt = Singleton with Int
```

We now have have a generic representation for a combination of dimensions.
...
This is harder to make inductive because there are two input lists involved.  Luckily, we only need to recurve over one of them, as we can pick dimensions from the other using shapeless's `Selector`.  We will recurse over the left list and can pick elements from the right list.
...
We can now have compile time diensionan analysis without writing out divide instances for every combination of dimensions:
...
```
Yay! We've solved the problem!  It looks a lot more verbose than what we started with, but we can tidy this up by using the ops pattern:
